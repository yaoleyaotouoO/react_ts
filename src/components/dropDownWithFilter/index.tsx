...
export class DropDownWithFilter extends React.Component<IDropDownWithFilterProps, IDropDownWithFilterState>{
  ...
  scroll: Scrollbars;
  dropDownNode: React.RefObject<HTMLDivElement>;
  ulNode: React.RefObject<HTMLDivElement>;
  ...
  componentDidMount() {
    this.hidePopupUtil = HidePopupUtil.init((this.dropDownNode.current), () => {
      if (this.state.open) {
...
const ContentLi = (props: IContentLiProps) => {
  ...
  return <React.Fragment>
    {
      sortData.filter(item => item.name.match(new RegExp(filterText, 'ig'))).slice(0, 300).map(item => {
